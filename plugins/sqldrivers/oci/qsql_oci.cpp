...
static inline QVariant::Type decodeNumber(int precision, int scale)
{
  if (scale > 0)
  {
...
QVariant::Type qDecodeOCIType(const QString& ocitype, int ocilen, int ociprecision, int ociscale)
{
    Q_UNUSED(ocilen);
...
QVariant::Type qDecodeOCIType(int ocitype, int precision, int scale)
{
    QVariant::Type type = QVariant::Invalid;
    switch (ocitype) {
    case SQLT_STR:
    ...
    case SQLT_FLT:
        ...
        break;
    case SQLT_NUM:
        type = decodeNumber(precision, scale);
        break;
    case SQLT_VNU:
    case SQLT_UIN:
        type = QVariant::Int;
...
OraFieldInfo QOCICols::qMakeOraField(const QOCIResultPrivate* p, OCIParam* param) const
{
    ...
    type = qDecodeOCIType(colType, colPrecision, colScale);
...
QStringList QOCIDriver::tables(QSql::TableType type) const
{
    ...
    // special case: ONLY tables/synonyms/views of current schema!
    if (type & 0x80) {
      QStringList tl;
      ...
      if (type & QSql::Tables) {
        // add tables to list
        QString query = QLatin1String("select table_name from user_tables");
        t.exec(query);
        ...
        query = QLatin1String("select synonym_name from user_synonyms");
        t.exec(query);
      ...
      if (type & QSql::Views) {
        // add views to list
        QString query = QLatin1String("select view_name from user_views");
        t.exec(query);
...
QSqlRecord QOCIDriver::record(const QString& tablename) const
{
    ...
    tmpStmt = stmt + QString(QLatin1String("where a.table_name=")) + QLatin1Char('\'') + table + QLatin1Char('\'');
    if (owner.isEmpty()) {
        owner = d->user;
    }
